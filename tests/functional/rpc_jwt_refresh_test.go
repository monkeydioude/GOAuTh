...
// 	layout.AccessTokenFactory.TimeFn = func() time.Time { return timeRef.Add(5 * time.Second) }
...
// 	jwt2, err := layout.AccessTokenFactory.GenerateToken(crypt.JWTDefaultClaims{
// 		UID:   11,
// 		Realm: login,
// 	})
// 	assert.NoError(t, err)
...
// 	})
// 	// enforcing AccessTokenFactory time creation date forward in time
// 	layout.AccessTokenFactory.TimeFn = func() time.Time { return timeRef.Add(12 * time.Second) }
// 	assert.NoError(t, err)
// 	client := v1.NewJWTClient(conn)
// 	ctx := context.Background()
...
func TestRefreshToken(t *testing.T) {
	...
	_, err := authClient.Signup(t.Context(), &v1.UserRequest{
		Login:    login,
		Password: "cabane123",
		Realm:    "test",
	})
	...
	_, err = authClient.Login(t.Context(), &v1.UserRequest{
		Login:    login,
		Password: "cabane123",
		Realm:    "test",
	}, grpc.Header(&loginMD))
	assert.NoError(t, err)
	client := v1.NewJWTClient(conn)
	jwtCookie, err := rpc.FetchCookie(loginMD, consts.RefreshTokenCookie)
	assert.NoError(t, err)
	var headerMD metadata.MD
	ctx := metadata.NewOutgoingContext(t.Context(), rpc.SetCookie(jwtCookie))
	layout.RefreshTokenFactory.TimeFn = func() time.Time { return timeRef }
	...
	assert.NoError(t, err)
	cookie, err := rpc.FetchCookie(headerMD, consts.RefreshTokenCookie)
	assert.NoError(t, err)
	assert.NotEqual(t, jwtCookie.Value, cookie.Value)
}
