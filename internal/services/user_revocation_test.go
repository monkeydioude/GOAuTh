...
func TestFindUserByLogin(t *testing.T) {
	...
	isRevoked, err := IsLoginRevoked[crypt.JWTDefaultClaims, entities.DefaultUser]("user_1_false", gormDB, timeRef)
	if err != nil {
		t.Error(err.Error())
	}
	if isRevoked == true {
	...
	isRevoked, err = IsLoginRevoked[crypt.JWTDefaultClaims, entities.DefaultUser]("user_2_false", gormDB, timeRef)
	if err != nil {
		t.Error(err.Error())
	}
	if isRevoked == true {
	...
	isRevoked, err = IsLoginRevoked[crypt.JWTDefaultClaims, entities.DefaultUser]("user_3_true", gormDB, timeRef)
	if err != nil {
		t.Error(err.Error())
	}
	if isRevoked == false {
