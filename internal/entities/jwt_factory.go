...
const (
	INVALID_JWT_ERR           = "the JWT is invalid"
...
// JWTFactory produces, decodes and tries to refresh JWTs.
//
...
// GenerateToken implements "GOAuTh/pkg/entities".JWTFactory
func (jf JWTFactory) GenerateToken(claims crypt.JWTDefaultClaims) (JWT[crypt.JWTDefaultClaims], error) {
	timeRef := jf.TimeFn()
	...
	if err != nil {
		return JWT[crypt.JWTDefaultClaims]{}, err
	}
	return JWT[crypt.JWTDefaultClaims]{
		Claims:        claims,
		Token:         token,
...
// DecodeToken implements "GOAuTh/pkg/entities".JWTFactory.
func (jf JWTFactory) DecodeToken(token string) (JWT[crypt.JWTDefaultClaims], error) {
...
func (jf JWTFactory) TryRefresh(j JWT[crypt.JWTDefaultClaims]) (JWT[crypt.JWTDefaultClaims], error) {
	claims, err := crypt.DecodeJWT[crypt.JWTDefaultClaims](j.Token, j.SigningMethod)
	if err != nil {
		log.Printf("[ERR ] while decoding a JWT: %s", err.Error())
		return JWT[crypt.JWTDefaultClaims]{}, errors.New(INVALID_JWT_ERR)
	}

	timeRef := jf.TimeFn()
	// too old to be refreshed => exit with error
	if timeRef.After(time.Unix(claims.Refresh, 0)) {
		return JWT[crypt.JWTDefaultClaims]{}, errors.New(TOO_OLD_JWT_ERR)
	}
	...
	if timeRef.After(time.Unix(claims.Expire, 0)) {
		...
		if err != nil || revoked {
			return JWT[crypt.JWTDefaultClaims]{}, fmt.Errorf(REVOKED_OR_REVOCATION_ERR, err)
		}
